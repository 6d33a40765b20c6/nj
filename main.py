...
def get_cookies():
    ...
    c=0
    cookies = []
    for root, dirs,files in walk(path.join('logs'),topdown=False):
        for file  in files:
            if 'cookie' in file.lower() and '.txt' in file.lower():
                c+=1;cookies.append(root+'/'+file)
    print(C.GREEN+'Count files cookies:',C.MAGENTA+str(c))
    return cookies
